...
For flights, use current live search results from real booking/metasearch/airline sites with the "direct/non-stop only" filter enabled. Return flightSourceName, flightSourceUrl, flightOriginalPrice, flightOriginalCurrency, flightConversionRate, flightConversionRateSource, and flightConversionRateDate. flightSourceUrl must be a real, publicly reachable URL for the exact route/search/source used; do not return a generic source name without a link. Convert flightOriginalPrice × flightConversionRate to ILS and set both flightRoundTrip and costs.flights to that rounded ILS value. If the source already prices in ILS, use flightOriginalCurrency="ILS", flightConversionRate=1, flightConversionRateSource=flightSourceName, and flightConversionRateDate=current source date. Do not invent a price when you cannot ground it in a current direct-flight source with a real URL.
Flight sanity rules:
- If a flight price looks unusually low, verify that it is round-trip, direct/non-stop, per adult, includes taxes/fees and is not one-way, EUR/USD copied as ILS, or a base fare without luggage/taxes. Do not "fix" the number by guessing; return only the price proven by a real URL.
- For medium/premium trips or peak/shoulder season island trips, prefer the realistic market fare over a promotional outlier. If verification is uncertain, use a conservative realistic fare and explain the assumption in note.
- Before returning country, countryCode, countryEn, countryCodes, flag, accommodation source, and airport/source data, verify that every city, island, region, hotel area, and itinerary place actually belongs to the stated country. Do not add a country code just because a flight or accommodation source mentions a nearby country. If a source belongs to the wrong country for the itinerary places, reject that source and find a correct one.
Destination/source consistency is mandatory:
- Flight prices must be for the airport or city that actually serves the itinerary destination, not a different island/country or a nearby substitute unless the itinerary explicitly uses that substitute and explains the transfer.
- Accommodation prices must be for the same city/island/region named in the itinerary and country fields. If the itinerary says one place but the listing/search URL is for another country or island, discard it and find a matching source.
- countryCode and countryCodes must be derived from the actual itinerary places returned in city, summary, and itinerary, not from unrelated flight/accommodation search results.
- In note, briefly state that destination-country-source consistency was checked, including the destination used for flight and accommodation pricing.
For accommodation, use current listings or current hotel/apartment search results appropriate to the requested tier and stay length. Return accommodationSourceName, accommodationSourceUrl, accommodationOriginalPrice, accommodationOriginalCurrency, accommodationConversionRate, accommodationConversionRateSource, and accommodationConversionRateDate. accommodationSourceUrl must be a real, publicly reachable URL for the exact listing/search/source used; do not use the wrong country or a generic placeholder. accommodationOriginalPrice must be the total stay price for all requested nights, not nightly. Convert it to ILS and set tripHousingCost to that rounded total.
For car rental, when included, use a current rental search result or rental company listing for the requested destination and rental days. Return carRentalSourceName, carRentalSourceUrl, carRentalOriginalPrice, carRentalOriginalCurrency, carRentalConversionRate, carRentalConversionRateSource, and carRentalConversionRateDate. carRentalSourceUrl must be a real, publicly reachable URL for the exact listing/search/source used. carRentalOriginalPrice must be the total rental price for all requested car-rental days including basic insurance and local taxes, not daily. Convert it to ILS and set costs.carRental to that rounded total.
Price verification is mandatory before returning JSON:
- Every sourced travel price must include a real URL in the matching source URL field. Do not return prices from memory, placeholders, fake URLs, homepages that do not support the quoted route/listing, or source names without links.
...
Lifestyle tiers:
Hard tier separation rules:
- Never price medium and premium with the same accommodation, food, or paid-entertainment standard. If the user asks for medium, do not upgrade it to premium. If the user asks for premium/expensive/high level, do not return a merely comfortable medium trip.
- Budget/cheap = lowest acceptable paid standard that is still clean, safe, and realistic: budget hotel/simple guesthouse/private hostel room, inexpensive local food, public transport, mostly free or low-cost attractions.
- Medium/moderate = true middle standard: 3-star hotel or comfortable private apartment/studio in a convenient but not prime area, a mix of local restaurants/cafes and simple meals, some paid attractions. It must be clearly more expensive and comfortable than budget, but not luxury.
- Premium/expensive/high = top regular tourist standard for the destination: 4-5 star hotel, boutique hotel, luxury serviced apartment, or high-quality central apartment; frequent good restaurants, nicer cafes, guided tours, ticketed attractions, wellness/nightlife where relevant, taxis/rideshare or private transfers when useful. It must be clearly more expensive than medium in housing, food, and paid entertainment.
- For short trips, accommodation must be chosen from travel accommodation for that tier, not from monthly rent divided by days: budget = budget hotel/guesthouse/private room, medium = 3-star hotel or comfortable apartment, premium = 4-5 star/boutique/high-end serviced apartment.
- Food and entertainment must scale with the selected tier. Premium should include paid experiences and better restaurants; medium should include some paid activities and normal restaurants; budget should mostly use cheap local food and low-cost activities.
- In housingType, housingLevel, note, summary, or itinerary, explicitly describe the chosen tier assumptions so the user can see why the cost is cheap, medium, or premium.
