export const translations = {
    en: {
        appTitle: "Retirement Planner",
        appSubtitle: "Visualize your financial future with tax-adjusted projections.",
        loadingWorkspace: "Preparing your retirement workspace...",
        parameters: "Parameters",
        birthdate: "Birthdate (Optional)",
        currentAge: "Current Age",
        startAge: "Start Age",
        endAge: "End Age",
        currentSavings: "Current Savings",
        targetEndBalance: "Target End Balance",
        targetBalanceVsIncome: "Target Balance vs Max Income",
        maxMonthlyIncome: "Max Monthly Income",
        monthlyContribution: "Monthly Contribution",
        monthlyNetIncomeDesired: "Desired Monthly Net Income",
        annualReturnRate: "Annual Return (%)",
        taxRate: "Tax Rate (%)",
        // ── Profile change history ──
        changeHistory: "Change History",
        historyTimeline: "Timeline",
        historyTrend: "Trend",
        historyCreated: "Created",
        historyUpdated: "Updated",
        historyChanged: "changed",
        historyMore: "more",
        historyField: "Field",
        historyDate: "Date",
        historyRestore: "Restore version",
        historyRestoreConfirm: "Restore this version's values into the form? It will show as unsaved until you update the profile.",
        historyDeleteConfirm: "Delete this version from history?",
        historyEmpty: "No history yet. Save or update the profile to record changes.",
        historyNoProfile: "Load or save a profile to see its change history.",
        historyNoTrend: "Not enough data to chart this field yet.",
        historyRetention: "Keep versions:",
        historyUnlimited: "Unlimited",
        showLess: "Show less",
        historySavingsBreakdownDesc: "Breakdown of savings growth between saves: how much came from contributions vs. actual return — and the return actually earned over that span against what the plan implies for the same span.",
        historyPeriodEnd: "Up to",
        historyMonths: "Months",
        historyDeltaSavings: "Δ Savings",
        historyReturnAmount: "Return ₪",
        historyAnnualizedReturn: "Annualized",
        historyPlannedReturn: "Planned",
        historyActualForPeriod: "Actual, this span",
        historyExpectedForPeriod: "Expected, same span",
        historyGap: "Gap",
        historyPerYearShort: "/yr",
        historyTotalSinceStart: "Overall",
        historyDateTime: "Date & time",
        historyEdit: "Edit version",
        historyEdited: "edited",
        historyActual: "Actual",
        historyExpectedPlan: "Per plan",
        balanceAtRetirement: "Balance at Retirement",
        projectedSavings: "Projected savings",
        requiredCapital: "Required Capital at Retirement Age",
        toEndWithZero: "To end with $0",
        surplus: "Surplus",
        deficit: "Additional Capital Needed at Retirement Age",
        onTrack: "You are on track!",
        capitalNeeded: "Additional capital needed",
        estGrossWithdrawal: "Est. Gross Withdrawal",
        withdrawalRateFromPortfolio: "Withdrawal Rate",
        maxSustainableWithdrawal: "Max for ₪0 at End",
        monthlyPreTaxNeeded: "Monthly pre-tax needed",
        capitalPreservation: "Capital for Perpetuity",
        preservePrincipal: "To live off interest only",
        wealthProjection: "Wealth Projection",
        warningRunOut: "Warning: Your savings are projected to run out at age",
        saveProfile: "Save Profile",
        loadProfile: "Load Profile",
        updateProfile: "Update Saved",
        profileName: "Profile Name",
        save: "Save",
        saveName: "Save Name",
        rename: "Rename",
        cancel: "Cancel",
        load: "Load",
        delete: "Delete",
        currency: "$",
        dir: "ltr",
        balanceAtEnd: "Balance at end of period",
        projectedBalanceAtAge: "Projected balance at age",
        gross: "Gross",
        yearsUntilRetirement: "Years until Retirement",
        yearsOfRetirement: "Years of Retirement",
        timeHorizon: "Time Horizon",
        neededToday: "Needed Today",
        accumulatedFromToday: "Accumulated from today",
        deposits: "Deposits",
        interest: "Interest",
        accumulatedWithdrawals: "Accumulated Withdrawals",
        signInWithGoogle: "Sign in with Google",
        logout: "Logout",
        loginErrorNetwork: "No internet connection. Please check your network and try again.",
        loginErrorTooMany: "Too many login attempts. Please wait a moment and try again.",
        loginErrorPopupBlocked: "The login popup was blocked. Please allow popups for this site.",
        loginErrorGeneral: "Login failed. Please try again.",
        createPinTitle: "Create App PIN",
        createPinDesc: "Create a PIN for this account. You will enter it each time this app opens after Google sign-in.",
        unlockPinTitle: "Unlock App",
        unlockPinDesc: "Enter the PIN for this Google account to open your retirement data.",
        loadingPin: "Loading PIN lock...",
        pinLabel: "PIN",
        confirmPinLabel: "Confirm PIN",
        savePin: "Save PIN",
        unlock: "Unlock",
        pinDigitsError: "PIN must contain 4 to 8 digits.",
        pinMismatchError: "PIN entries do not match.",
        pinIncorrectError: "Incorrect PIN.",
        pinLoadError: "Could not load the PIN lock.",
        pinSaveError: "Could not save the PIN.",
        pinVerifyError: "Could not verify the PIN.",
        pinLockedError: "Too many failed attempts. Try again in {seconds}s.",
        pinAttemptCount: "Attempt {current} of {max}",
        pinAttemptsLeft: "{remaining} attempts left before lockout",
        pinLastAttempt: "Last attempt before lockout!",
        pinRememberWarning: "Remember this PIN. It is stored as a verifier, not as readable text.",
        pinUnlockHint: "This is an extra app lock after Google sign-in.",
        changePinTitle: "Change App PIN",
        changePinDesc: "Enter the current PIN before saving a new one.",
        currentPinLabel: "Current PIN",
        newPinLabel: "New PIN",
        changePinAction: "Change PIN",
        pinChanged: "PIN changed.",
        clearPinFields: "Clear",
        screenLockedTitle: "Screen Locked",
        screenLockedDesc: "Enter your PIN to return to your open session.",
        screenLockAfterInactivity: "Lock screen after inactivity",
        screenLockBeforeLogoutHint: "Screen lock must occur before auto-logout. Unlocking uses the app PIN.",
        screenLockRequiresLogoutHint: "Enable auto-logout for more than 1 minute to use screen lock.",

        // AI & Simulations
        calculationMode: 'Calculation Mode',
        mathematical: 'Mathematical',
        aiMode: 'AI',
        simulations: 'Simulations',
        compare: 'Compare',
        planningMode: 'Checklist',
        selectProvider: 'Select Provider',
        selectModel: 'Select Model',
        simulationType: 'Simulation Type',
        monteCarlo: 'Monte Carlo (Ranges)',
        conservative: 'Conservative',
        optimistic: 'Optimistic',
        calculatingWithAI: 'Calculating with AI...',
        aiError: 'AI Calculation Error',
        mathResults: 'Mathematical Results',
        aiResults: 'AI Results',
        simResults: 'Simulation Results',
        difference: 'Difference',
        calculatedBy: 'Calculated by',
        upToDate: 'Up to date',
        generate: 'Generate',
        calculateAI: 'Calculate with AI',
        medianResult: 'Median Result',
        range: 'Range (P25-P75)',
        apiKeyMissing: 'API Key Missing',
        sensitivityInsight: 'Key Insight: What Moves the Needle?',
        metric: 'Metric',
        conservativeDesc: 'Annual Return -2%',
        optimisticDesc: 'Annual Return +1.5%',
        monteCarloDesc: '500 runs, realistic market volatility',
        selectProfiles: 'Select Profiles',
        selectProfilesToCompare: 'Select Profiles to Compare',
        noSavedProfiles: 'No saved profiles found. Save a profile to compare.',
        basicScenarios: 'Basic Profiles',
        interestSensitivity: 'Interest (+/- 1%)',
        returnMinus1: 'Return -1%',
        returnPlus1: 'Return +1%',
        incomeSensitivity: 'Income (+/- 1000)',
        incomeMinus1000: 'Income -1000',
        incomePlus1000: 'Income +1000',
        ageSensitivity: 'Retirement (±1 yr)',
        retireMinus1: 'Retire -1y',
        retirePlus1: 'Retire +1y',
        sensitivityAnalysis: 'Sensitivity Analysis',
        sensitivityRangeBtn: 'Range',
        sensitivityHeatmapBtn: 'Heatmap',
        currentInputs: 'Current',
        provider: 'Provider',
        model: 'Model',
        // Amortization table
        showYearlyProgress: 'Year-by-Year Progress',
        year: 'Year',
        balance: 'Balance',
        contributionHeader: 'Contribution',
        withdrawalHeader: 'Withdrawal',
        interestEarned: 'Interest Earned',
        age: 'Age',
        // Sensitivity Range Chart
        sensitivityRangeChart: 'Sensitivity Range Chart',
        selectParameter: 'Parameter',
        rangeMin: 'Min',
        rangeMax: 'Max',
        step: 'Step',
        interestRate: 'Interest Rate',
        monthlyIncome: 'Monthly Income',
        retirementAge: 'Retirement Age',
        currentValue: 'Current Value',
        balanceAtEndShort: 'End Balance',
        requiredWithdrawal: 'Required Withdrawal',
        years: 'years',
        mostImpactfulParameter: 'Most Impactful Factor',
        mostImpactfulFactor: 'Top Factor',
        mostImpactfulRate: 'Top Rate',
        averageChange: 'Avg Change',
        impactAnalysis: 'Impact Analysis',
        // Sensitivity Heatmap
        sensitivityHeatmap: 'Sensitivity Heatmap',
        heatmapDesc: 'Balance at End of Period (Green = Surplus, Red = Deficit)',
        heatmapXAxis: 'Annual Return Rate (%)',
        heatmapYAxis: 'Monthly Income Desired',
        reset: 'Reset View',
        heatmapLowerIncome: 'Lower Income',
        heatmapHigherIncome: 'Higher Income',
        heatmapLowerReturn: 'Lower Return',
        heatmapHigherReturn: 'Higher Return',
        heatmapScenarioControls: 'Age Scenarios',
        adjustRetirementAge: 'Retirement Age',
        adjustRetirementEndAge: 'End Age',
        heatmapLegend: 'Legend',
        livingOffInterest: 'Living off Interest',
        surplusHigh: '> 2M',
        surplusMedium: '0.5M - 2M',
        surplusLow: '< 0.5M',
        // Inflation Reality Check
        inflationRealityCheckTitle: 'Inflation Time Machine',
        inflationRealityCheckDesc: 'Your "Budget" in {years} years...',
        inflationDisclaimer: 'Projected prices based on inflation rate of',
        item_coffee: 'Cappuccino',
        item_bread: 'Loaf of Bread',
        item_movie: 'Cinema Ticket',
        item_beer: 'Pint of Beer',
        item_car: 'Family Sedan',
        item_vacation: 'Family Vacation',
        inflationRate: 'Inflation (%)',
        additionalIncome: 'Additional Income',
        additionalIncomeDesc: 'Net monthly income that reduces portfolio withdrawal in those years',
        addIncomeEntry: 'Add Year',
        incomeEntryFrom: 'From',
        incomeEntryTo: 'To (optional)',
        incomeEntryAmount: 'Monthly Net',
        noAdditionalIncome: 'No additional income configured',
        inflationBtn: 'Inflation',
        yearsToFuture: 'Years into the Future',
        // Pension Income
        pensionIncome: 'Pension Income',
        pensionIncomeBtn: 'Pension',
        pensionIncomeDesc: 'Retirement income summary by age',
        capitalAtRetirement: 'Capital at Retirement',
        totalGrossPension: 'Total Gross Pension',
        totalNetPension: 'Total Net Pension',
        monthlyExpenses: 'Monthly Expenses',
        incomeSources: 'Income Sources',
        addPension: 'Pension',
        addRent: 'Rent',
        addOther: 'Other',
        totalAccumulatedCapital: 'Total Accumulated Capital',
        noIncomeSources: 'No income sources defined',
        incomeByAge: 'Income by Age',
        fromAge: 'From age',
        toAge: 'Until age',
        autoCalculated: 'Auto',
        effectiveTaxRate: 'Effective Tax Rate',
        capitalLastsUntil: 'Capital lasts until age',
        activeIncomeSources: 'Active income sources',
        addAnnuity: 'Add Annuity',
        addCapital: 'Add Capital',
        accumulatedCapitalAtAge: 'Accumulated Capital',
        netIncome: 'Net',
        balanced: 'Balanced',
        pensionDisclaimer: 'Estimates only (2026)',
        interestOnlyIncome: 'Interest-Only Income',
        pensionInterestRate: 'Interest Rate',
        fiscalSettings: 'Fiscal Settings',
        searchingFiscalData: 'Searching for {year} fiscal data...',
        validatingFiscalDataAttempt: 'Attempt {attempt} - validating fiscal data...',
        analyzingFiscalSources: 'Analyzing data and cross-referencing sources...',
        updatingOfficialFiscalSources: 'Updating from official current-year sources...',
        loadingVerifiedFiscalSources: 'Loading verified {year} fiscal sources...',
        officialWebSources: 'Data from official web sources:',
        taxAuthorityVerifiedFiscalDefaults: 'Tax brackets and qualifying pension exemption use {year} values verified against the Tax Authority booklet.',
        nationalInsuranceVerifiedFiscalFallback: 'The live National Insurance fetch did not complete; verified {year} official-source values were loaded.',
        aiTrainingDataNoWebGeminiKey: 'Data from AI training data - no web search performed. Add a Gemini API key in settings to enable real-time search.',
        aiTrainingDataNoWebGrounding: 'Data from AI training data - no web search performed. The current model does not support Google Search grounding.',
        fiscalSourceBtlRates: 'btl.gov.il - old-age pension rates {year}',
        fiscalSourceBtlIncomeTest: 'btl.gov.il - work income test {year}',
        fiscalSourceTaxBooklet: 'Tax Authority - monthly deductions booklet {year}',
        viewTaxBrackets: 'View Tax Brackets',
        close: 'Close',
        // Withdrawal Strategies
        withdrawalStrategy: 'Withdrawal Strategy',
        withdrawalFixed: 'Fixed Amount',
        withdrawalFourPercent: '4% Rule',
        withdrawalPercentage: '% of Portfolio',
        withdrawalDynamic: 'Dynamic',
        withdrawalFixedDesc: 'Fixed monthly withdrawal amount',
        variableRatesDesc: 'Return rate varies by year',
        withdrawalFourPercentDesc: '4% of initial balance per year',
        fourPercentModeLabel: '4% Rule Mode',
        fourPercentModeNet: 'Net (take-home = 4%)',
        fourPercentModeGross: 'Gross (portfolio depletes at 4%)',
        fourPercentModeNetDesc: 'You receive 4%/year net after tax. Portfolio actually depletes faster.',
        fourPercentModeGrossDesc: 'Portfolio withdraws exactly 4% gross/year. You receive less after capital gains tax.',
        withdrawalPercentageDesc: 'Percentage of current balance',
        withdrawalDynamicDesc: 'Adjusts based on market performance',
        withdrawalInterestOnly: 'Interest Only',
        withdrawalInterestOnlyDesc: 'Withdraws the portfolio growth earned each month, after tax. It can pay more than your target, but leaves less growth to compound.',
        withdrawalGuardrails: 'Guardrails',
        withdrawalGuardrailsDesc: 'Guyton-Klinger: adjusts ±10% when portfolio withdrawal rate crosses ±20% of initial',
        withdrawalVpw: 'VPW',
        withdrawalVpwDesc: 'Variable Percentage Withdrawal: balance ÷ months remaining — adjusts automatically to lifespan',
        withdrawalCape: 'CAPE',
        withdrawalCapeDesc: 'CAPE-based: withdrawal rate tied to Shiller P/E — lower when markets are expensive, higher when cheap',
        withdrawalPercentageRate: 'Withdrawal Rate (%)',
        netWithdrawal: 'Net Withdrawal',
        // Error Boundary
        displayError: 'Display Error',
        unknownError: 'Unknown error',
        tryAgain: 'Try Again',
        chunkLoadError: 'A new version of the app is available — please reload the page',
        reloadPage: 'Reload Page',

        // Bucket Strategy
        bucketStrategy: 'Bucket Strategy',
        enableBuckets: 'Enable Separate Buckets',
        accumulationRate: 'Accumulation Rate (Pre-Retirement)',
        safeRate: 'Safe Bucket Rate',
        surplusRate: 'Surplus Bucket Rate',
        safeBucketDesc: 'Capital reserved for essential income',
        surplusBucketDesc: 'Capital for growth/legacy',
        safeBucketBalance: 'Safe Bucket',
        surplusBucketBalance: 'Surplus Bucket',

        // Life Events Timeline
        lifeEventsTimeline: 'Life Events Timeline',
        addEvent: 'Add Event',
        editEvent: 'Edit Event',
        noEventsYet: 'No life events added yet. Click "Add" to create one.',
        eventType: 'Event Type',
        oneTimeIncome: 'One-Time Income',
        oneTimeExpense: 'One-Time Expense',
        incomeChange: 'Income Change',
        expenseChange: 'Expense Change',
        startDate: 'Start Date',
        endDate: 'End Date',
        amount: 'Amount',
        monthlyChange: 'Monthly Change',
        description: 'Description',
        descriptionPlaceholder: 'e.g., Inheritance, Car Purchase, Salary Increase...',
        enableEvent: 'Enable Event',
        disableEvent: 'Disable Event',
        deleteEvent: 'Delete Event',
        editEventAction: 'Edit Event',
        deleteAll: 'Delete All Events',
        month: 'Month',
        january: 'January',
        february: 'February',
        march: 'March',
        april: 'April',
        may: 'May',
        june: 'June',
        july: 'July',
        august: 'August',
        september: 'September',
        october: 'October',
        november: 'November',
        december: 'December',
        hasEndDate: 'Specific End Date (uncheck for permanent)',
        positiveForIncrease: 'Positive for increase (+), negative for decrease (-)',
        saveChanges: 'Save Changes',
        pleaseSelectStartDate: 'Please select a start date',
        pleaseSelectEndDate: 'Please select an end date',
        pleaseEnterAmount: 'Please enter a valid amount',
        pleaseEnterMonthlyChange: 'Please enter a valid monthly change',
        perMonth: '/month',
        viewTimeline: 'Timeline',
        ongoing: 'Ongoing',

        // AI Error Messages
        errorRateLimit: 'Rate limit exceeded. Please try again later or use a different model.',
        errorInvalidApiKey: 'Invalid API key. Please check your credentials in settings.',
        errorModelNotFound: 'Model "{model}" not found for {provider}. Please select a different model.',
        errorNetwork: 'Network error. Please check your internet connection and try again.',
        errorJsonParse: 'Failed to parse AI response. The model may have returned invalid data. Please try again or use a different model.',
        errorMissingApiKey: 'Missing API key for provider: {provider}. Please configure your .env file or provide an API key override in settings.',
        errorGeneric: 'AI calculation failed: {error}. Please try again or switch to mathematical mode.',

        // Validation Error Messages
        validationInvalidInputs: 'Invalid inputs:',
        validationCurrentAgeBetween: 'Current age must be between 0 and 120',
        validationRetirementStartAgeBetween: 'Retirement start age must be between 0 and 120',
        validationRetirementEndAgeBetween: 'Retirement end age must be between 0 and 120',
        validationRetirementStartGreater: 'Retirement start age must be greater than or equal to current age',
        validationRetirementEndGreater: 'Retirement end age must be greater than retirement start age',
        validationCurrentSavingsNonNegative: 'Current savings cannot be negative',
        validationMonthlyContributionNonNegative: 'Monthly contribution cannot be negative',
        validationMonthlyIncomePositive: 'Monthly net income desired must be positive',
        validationAnnualReturnBetween: 'Annual return rate must be between -100% and 100%',
        validationTaxRateBetween: 'Tax rate must be between 0% and 100%',

        // Rate Limit Messages
        rateLimitMinute: 'Rate limit: 1 call per minute. Please wait {seconds} seconds.',
        rateLimitHour: 'Rate limit: {limit} calls per hour. Try again at {time}.',
        rateLimitDay: 'Daily limit reached ({limit} calls). Resets tomorrow at midnight.',
        rateLimitReached: 'Rate limit reached',
        callsToday: 'calls today',
        usageStats: 'AI Usage: {today}/{dailyLimit} today',

        // Models Manager
        manageModels: 'Settings',
        modelsManagerDesc: 'Check for available models from each AI provider and update your list.',
        checkModels: 'Check Models',
        checking: 'Checking...',
        applyUpdates: 'Apply Updates',
        resetToDefault: 'Reset to Default',
        modelsUpdated: 'Models updated successfully!',
        modelsReset: 'Models reset to default!',
        updatesAvailable: 'Updates Available',
        existing: 'Existing',
        new: 'New',
        removed: 'Removed',
        hideDetails: '▼ Hide Details',
        showDetails: '▶ Show Details',
        newModels: 'New Models:',
        removedModels: 'Removed Models:',

        // Header Buttons
        switchToLightMode: 'Switch to Light Mode',
        switchToDarkMode: 'Switch to Dark Mode',
        selected: 'selected',
        selectAll: 'Select All',
        deselectAll: 'Deselect All',

        // AI Insights View
        numericalResults: 'Numerical Results',
        aiInsights: 'AI Insights',
        budget: 'Budget',
        budgetIncomeTarget: 'Monthly income target',
        budgetTotalExpenses: 'Total expenses',
        budgetGap: 'Gap',
        budgetOfTarget: 'of target',
        budgetAddItem: 'Add item',
        budgetNewItem: 'New item',
        budgetAddCategory: 'Add category',
        budgetCategoryName: 'Category name:',
        budgetAdoptTarget: 'Set as income target',
        budgetReset: 'Reset',
        budgetResetConfirm: 'Reset all amounts to zero?',
        budgetMonthly: 'Mo',
        budgetAnnual: 'Yr',
        budgetClickToEdit: 'Click to edit',
        budgetAddLoan: 'Add Loan / Mortgage',
        budgetTrack: 'Track',
        budgetAddTrack: 'Add track',
        budgetEndDate: 'End',
        budgetLoanDone: 'Done',
        getAiInsights: 'Get AI Insights',
        getAiInsightsDesc: 'Use AI to analyze your financial situation and get personalized recommendations.',
        analyzeNow: 'Analyze Now',
        processingInsights: 'Processing data and generating insights...',
        readinessScore: 'Readiness Score',
        strengths: 'Strengths',
        attentionNeeded: 'Attention Needed',
        marketDependency: 'Market Dependency',
        recommendations: 'Recommendations',
        impact: 'Impact',
        regenerateInsights: 'Regenerate Insights',
        errorDetails: 'Error Details',
        errorHighTraffic: 'High Traffic (Rate Limit)',
        errorHighTrafficDesc: 'The model is currently busy. Please try "Gemini Flash" or wait a moment.',
        errorApiKey: 'API Key Issue',
        errorApiKeyDesc: 'Please check your API key settings.',
        aiRecommendedAge: 'AI Recommended Retirement Age',
        current: 'Current Plan',
        copyEventToProfile: 'Copy Event to Profile',
        selectTargetProfile: 'Select Target Profile:',
        selectTargetProfilePlaceholder: '-- Select Target Profile --',
        performCopy: 'Copy',
        copyEventError: 'Error copying event',
        manual: 'Manual',
        variableRates: 'Variable Rates',
        invalidAge: 'Invalid age',
        duplicateEventError: 'Error: An event with this exact name and type already exists',
        updateError: 'Update Error',
        unusualValuesWarning: 'Warning: Unusual Values',
        unusualValuesDesc: 'AI returned values significantly different from defaults. Verify before saving.',
        invalidAiResponse: 'Invalid AI response structure',
    },
    he: {
        loadingWorkspace: "מכין את סביבת התכנון שלך...",
        appTitle: "מתכנן פרישה",
        appSubtitle: "המחשה של העתיד הפיננסי שלך עם התאמות מס.",
        parameters: "פרמטרים",
        birthdate: "תאריך לידה (אופציונלי)",
        currentAge: "גיל נוכחי",
        startAge: "גיל פרישה",
        endAge: "גיל סיום",
        currentSavings: "חסכונות כרגע",
        targetEndBalance: "יעד יתרה בסיום",
        targetBalanceVsIncome: "יעד יתרה לעומת הכנסה חודשית מקסימלית",
        maxMonthlyIncome: "הכנסה חודשית מקסימלית",
        monthlyContribution: "הפקדה חודשית",
        monthlyNetIncomeDesired: "הכנסה חודשית נטו רצויה",
        annualReturnRate: "תשואה שנתית (%)",
        inflationRate: "אינפלציה (%)",
        // ── Profile change history ──
        changeHistory: "היסטוריית שינויים",
        historyTimeline: "ציר זמן",
        historyTrend: "מגמה",
        historyCreated: "נוצר",
        historyUpdated: "עודכן",
        historyChanged: "שונה",
        historyMore: "שינויים נוספים",
        historyField: "שדה",
        historyDate: "תאריך",
        historyRestore: "שחזר גרסה",
        historyRestoreConfirm: "לשחזר את הערכים מגרסה זו לטופס? השינוי יסומן כלא-שמור עד שתעדכן את הפרופיל.",
        historyDeleteConfirm: "למחוק גרסה זו מההיסטוריה?",
        historyEmpty: "אין עדיין היסטוריה. שמור או עדכן את הפרופיל כדי לתעד שינויים.",
        historyNoProfile: "טען או שמור פרופיל כדי לראות היסטוריית שינויים.",
        historyNoTrend: "אין מספיק נתונים להצגת מגמה עבור שדה זה.",
        historyRetention: "שמירת גרסאות:",
        historyUnlimited: "ללא הגבלה",
        showLess: "הצג פחות",
        historySavingsBreakdownDesc: "פירוק עליית החיסכון בין שמירות: כמה מהעלייה מקורו בהפקדות וכמה בתשואה בפועל — והתשואה שהושגה בפועל באותו פרק זמן מול מה שהתוכנית מבטיחה לאותו פרק זמן.",
        historyPeriodEnd: "עד תאריך",
        historyMonths: "חודשים",
        historyDeltaSavings: "Δ חיסכון",
        historyReturnAmount: "תשואה ₪",
        historyAnnualizedReturn: "תשואה שנתית",
        historyPlannedReturn: "מתוכננת",
        historyActualForPeriod: "בפועל לתקופה",
        historyExpectedForPeriod: "צפוי לתקופה",
        historyGap: "פער",
        historyPerYearShort: "שנתי",
        historyTotalSinceStart: "סה״כ",
        historyDateTime: "תאריך ושעה",
        historyEdit: "ערוך גרסה",
        historyEdited: "נערך",
        historyActual: "בפועל",
        historyExpectedPlan: "לפי התוכנית",
        additionalIncome: 'הכנסות נוספות',
        additionalIncomeDesc: 'הכנסה חודשית נטו שתופחת מהמשיכה מהתיק בשנים אלו',
        addIncomeEntry: 'הוסף שנה',
        incomeEntryFrom: 'משנת',
        incomeEntryTo: 'עד שנת (אופציונלי)',
        incomeEntryAmount: 'הכנסה חודשית נטו',
        noAdditionalIncome: 'לא הוגדרו הכנסות נוספות',
        taxRate: "מס רווחי הון (%)",
        balanceAtRetirement: "יתרה בפרישה",
        projectedSavings: "חסכונות צפויים",
        requiredCapital: "הון נדרש בגיל פרישה",
        toEndWithZero: "כדי לסיים עם 0 ש\"ח",
        surplus: "עודף",
        deficit: "הון נוסף נדרש בגיל פרישה",
        onTrack: "אתה במסלול הנכון!",
        capitalNeeded: "הון נוסף נדרש",
        estGrossWithdrawal: "משיכה ברוטו משוערת חודשית",
        withdrawalRateFromPortfolio: "אחוז משיכה מהתיק",
        maxSustainableWithdrawal: "מקסימום לסיום עם ₪0",
        monthlyPreTaxNeeded: "הכנסה חודשית ברוטו נדרשת",
        capitalPreservation: "הון למחייה מריבית בלבד",
        preservePrincipal: "כדי לחיות מריבית בלבד",
        wealthProjection: "תחזית עושר",
        warningRunOut: "אזהרה: החסכונות צפויים להסתיים בגיל",
        saveProfile: "שמור פרופיל",
        loadProfile: "טען פרופיל",
        updateProfile: "עדכן שמור",
        profileName: "שם פרופיל",
        save: "שמור",
        saveName: "שמור שם",
        rename: "שנה שם",
        cancel: "ביטול",
        load: "טען",
        delete: "מחק",
        currency: "₪",
        dir: "rtl",
        balanceAtEnd: "יתרה בסוף התקופה",
        projectedBalanceAtAge: "יתרה צפויה בגיל",
        gross: "ברוטו",
        yearsUntilRetirement: "שנים עד הפרישה",
        yearsOfRetirement: "שנות פרישה",
        timeHorizon: "אופק זמן",
        neededToday: "נדרש היום",
        accumulatedFromToday: "נצבר מהיום",
        deposits: "הפקדות",
        interest: "ריבית",
        accumulatedWithdrawals: "משיכות מצטברות",
        signInWithGoogle: "התחבר עם גוגל",
        logout: "התנתק",
        loginErrorNetwork: "אין חיבור לאינטרנט. בדוק את הרשת ונסה שוב.",
        loginErrorTooMany: "יותר מדי ניסיונות כניסה. המתן רגע ונסה שוב.",
        loginErrorPopupBlocked: "החלון הקופץ נחסם. אפשר חלונות קופצים לאתר זה.",
        loginErrorGeneral: "הכניסה נכשלה. נסה שוב.",
        createPinTitle: "יצירת PIN לאפליקציה",
        createPinDesc: "הגדר PIN לחשבון הזה. תתבקש להזין אותו בכל פתיחה אחרי התחברות עם Google.",
        unlockPinTitle: "פתיחת האפליקציה",
        unlockPinDesc: "הזן את ה-PIN של חשבון Google הזה כדי לפתוח את נתוני הפרישה.",
        loadingPin: "טוען נעילת PIN...",
        pinLabel: "PIN",
        confirmPinLabel: "אימות PIN",
        savePin: "שמור PIN",
        unlock: "פתח",
        pinDigitsError: "ה-PIN חייב להכיל 4 עד 8 ספרות.",
        pinMismatchError: "ה-PIN שהוזן לא זהה.",
        pinIncorrectError: "PIN שגוי.",
        pinLoadError: "לא ניתן לטעון את נעילת ה-PIN.",
        pinSaveError: "לא ניתן לשמור את ה-PIN.",
        pinVerifyError: "לא ניתן לאמת את ה-PIN.",
        pinLockedError: "יותר מדי ניסיונות כושלים. נסה שוב בעוד {seconds} שניות.",
        pinAttemptCount: "ניסיון {current} מתוך {max}",
        pinAttemptsLeft: "נותרו {remaining} ניסיונות לפני נעילה",
        pinLastAttempt: "ניסיון אחרון לפני נעילה!",
        pinRememberWarning: "חשוב לזכור את ה-PIN. הוא נשמר כמאמת ולא כטקסט קריא.",
        pinUnlockHint: "זו שכבת נעילה נוספת אחרי ההתחברות עם Google.",
        changePinTitle: "החלפת PIN לאפליקציה",
        changePinDesc: "יש להזין את ה-PIN הנוכחי לפני שמירת PIN חדש.",
        currentPinLabel: "PIN נוכחי",
        newPinLabel: "PIN חדש",
        changePinAction: "החלף PIN",
        pinChanged: "ה-PIN הוחלף.",
        clearPinFields: "נקה",
        screenLockedTitle: "המסך נעול",
        screenLockedDesc: "הזן PIN כדי לחזור לפעילות הפתוחה.",
        screenLockAfterInactivity: "נעילת מסך לאחר חוסר פעילות",
        screenLockBeforeLogoutHint: "הנעילה חייבת להיות לפני הניתוק האוטומטי. הפתיחה נעשית עם ה-PIN של האפליקציה.",
        screenLockRequiresLogoutHint: "יש להפעיל ניתוק אוטומטי ליותר מדקה כדי להשתמש בנעילת מסך.",

        // AI & Simulations
        calculationMode: 'מצב חישוב',
        mathematical: 'מתמטי',
        aiMode: 'AI',
        simulations: 'סימולציות',
        compare: 'השוואה',
        planningMode: 'צ\'קליסט',
        selectProvider: 'בחר ספק',
        selectModel: 'בחר מודל',
        simulationType: 'סוג סימולציה',
        monteCarlo: 'Monte Carlo (טווחים)',
        conservative: 'שמרני',
        optimistic: 'אופטימי',
        calculatingWithAI: 'מחשב עם AI...',
        aiError: 'שגיאה בחישוב AI',
        mathResults: 'תוצאות מתמטיות',
        aiResults: 'תוצאות AI',
        simResults: 'תוצאות סימולציה',
        difference: 'הבדל',
        calculatedBy: 'חושב על ידי',
        upToDate: 'מעודכן',
        generate: 'חשב',
        calculateAI: 'חשב עם AI',
        medianResult: 'תוצאה חציונית',
        range: 'טווח (P25-P75)',
        apiKeyMissing: 'חסר מפתח API',
        sensitivityInsight: 'תובנת מפתח: מה באמת משפיע?',
        metric: 'מדד',
        conservativeDesc: 'תשואה שנתית 2%-',
        optimisticDesc: 'תשואה שנתית 1.5%+',
        monteCarloDesc: '500 הרצות, תנודתיות שוק ריאליסטית',
        selectProfiles: 'בחר פרופילים',
        selectProfilesToCompare: 'בחר פרופילים להשוואה',
        noSavedProfiles: 'לא נמצאו פרופילים שמורים. שמור פרופיל כדי להשוות.',
        basicScenarios: 'פרופילי בסיס',
        interestSensitivity: 'ריבית (+/- 1%)',
        returnMinus1: 'תשואה 1%-',
        returnPlus1: 'תשואה 1%+',
        incomeSensitivity: 'הכנסה (+/- 1000)',
        incomeMinus1000: 'הכנסה 1000-',
        incomePlus1000: 'הכנסה 1000+',
        ageSensitivity: 'גיל פרישה (±1)',
        retireMinus1: 'פרישה -1 שנה',
        retirePlus1: 'פרישה +1 שנה',
        sensitivityAnalysis: 'ניתוח רגישות',
        sensitivityRangeBtn: 'טווח',
        sensitivityHeatmapBtn: 'מפת חום',
        currentInputs: 'נוכחי',
        provider: 'ספק',
        model: 'מודל',
        // Amortization table
        showYearlyProgress: 'התקדמות שנה אחרי שנה',
        year: 'שנה',
        balance: 'יתרה',
        contributionHeader: 'הפקדה',
        withdrawalHeader: 'משיכה',
        interestEarned: 'ריבית',
        age: 'גיל',
        // Sensitivity Range Chart
        sensitivityRangeChart: 'גרף טווח רגישות',
        selectParameter: 'פרמטר',
        rangeMin: 'מינימום',
        rangeMax: 'מקסימום',
        step: 'צעד',
        interestRate: 'ריבית שנתית',
        monthlyIncome: 'הכנסה חודשית',
        retirementAge: 'גיל פרישה',
        currentValue: 'ערך נוכחי',
        balanceAtEndShort: 'יתרה סופית',
        requiredWithdrawal: 'משיכה נדרשת',
        years: 'שנים',
        mostImpactfulParameter: 'הגורם המשפיע ביותר',
        mostImpactfulFactor: 'הגורם המשפיע',
        mostImpactfulRate: 'הריבית המשפיעה',
        averageChange: 'ממוצע שינוי לצעד',
        impactAnalysis: 'ניתוח רגישות',
        // Sensitivity Heatmap
        sensitivityHeatmap: 'מפת חום - רגישות',
        heatmapDesc: 'יתרה בסוף התקופה (ירוק = עודף, אדום = חוסר)',
        heatmapXAxis: 'תשואה שנתית (%)',
        heatmapYAxis: 'הכנסה חודשית רצויה',
        reset: 'אפס תצוגה',
        heatmapLowerIncome: 'הכנסה נמוכה יותר',
        heatmapHigherIncome: 'הכנסה גבוהה יותר',
        heatmapLowerReturn: 'תשואה נמוכה יותר',
        heatmapHigherReturn: 'תשואה גבוהה יותר',
        heatmapScenarioControls: 'תרחישי גיל',
        adjustRetirementAge: 'גיל פרישה',
        adjustRetirementEndAge: 'עד גיל',
        heatmapLegend: 'מקרא',
        livingOffInterest: 'מחיה מריבית',
        surplusHigh: '> 2M',
        surplusMedium: '0.5M - 2M',
        surplusLow: '< 0.5M',
        // Inflation Reality Check
        inflationRealityCheckTitle: 'מכונת זמן אינפלציונית',
        inflationRealityCheckDesc: 'ה"תקציב" שלך בעוד {years} שנים...',
        inflationDisclaimer: 'מחירים חזויים מבוססים על אינפלציה של',
        item_coffee: 'כוס קפה',
        item_bread: 'כיכר לחם',
        item_movie: 'כרטיס לקולנוע',
        item_beer: 'בירה בבר',
        item_car: 'רכב משפחתי',
        item_vacation: 'חופשה משפחתית',
        inflationBtn: 'אינפלציה',
        yearsToFuture: 'שנים לעתיד',
        // Pension Income
        pensionIncome: 'הכנסות פנסיוניות',
        pensionIncomeBtn: 'פנסיה',
        pensionIncomeDesc: 'סיכום הכנסות בפרישה לפי גיל',
        capitalAtRetirement: 'הון בסיום התקופה',
        totalGrossPension: 'סה"כ קצבה ברוטו',
        totalNetPension: 'סה"כ קצבה נטו',
        monthlyExpenses: 'הוצאות חודשיות',
        incomeSources: 'מקורות הכנסה',
        addPension: 'פנסיה',
        addRent: 'שכירות',
        addOther: 'אחר',
        totalAccumulatedCapital: 'הון צבור כולל',
        noIncomeSources: 'לא הוגדרו מקורות הכנסה',
        incomeByAge: 'הכנסה לפי גיל',
        fromAge: 'מגיל',
        toAge: 'עד גיל',
        autoCalculated: 'אוטומטי',
        effectiveTaxRate: 'מס אפקטיבי',
        capitalLastsUntil: 'הון יספיק עד גיל',
        activeIncomeSources: 'מקורות הכנסה פעילים',
        addAnnuity: 'הוסף קצבה',
        addCapital: 'הוסף הון',
        accumulatedCapitalAtAge: 'הון צבור',
        netIncome: 'נטו',
        balanced: 'מאוזן',
        pensionDisclaimer: 'הערכה בלבד (2026)',
        interestOnlyIncome: 'קצבה מריבית בלבד',
        pensionInterestRate: 'ריבית',
        fiscalSettings: 'הגדרות מיסוי',
        searchingFiscalData: 'מחפש נתונים פיסקליים לשנת {year}...',
        validatingFiscalDataAttempt: 'ניסיון {attempt} - מאמת נתונים פיסקליים...',
        analyzingFiscalSources: 'מנתח נתונים ומצליב מקורות...',
        updatingOfficialFiscalSources: 'מעדכן ממקורות רשמיים לשנת המס הנוכחית...',
        loadingVerifiedFiscalSources: 'טוען מקורות פיסקליים מאומתים לשנת {year}...',
        officialWebSources: 'נתונים ממקורות רשת רשמיים:',
        taxAuthorityVerifiedFiscalDefaults: 'מדרגות המס והפטור המזכה נטענו מערכי {year} המאומתים מול לוח רשות המסים.',
        nationalInsuranceVerifiedFiscalFallback: 'הקריאה החיה לביטוח לאומי לא הושלמה; נטענו ערכי {year} המאומתים מול המקורות הרשמיים.',
        aiTrainingDataNoWebGeminiKey: 'נתונים מנתוני האימון של ה-AI - לא נעשה חיפוש רשת. הוסף מפתח Gemini בהגדרות כדי לאפשר חיפוש בזמן אמת.',
        aiTrainingDataNoWebGrounding: 'נתונים מנתוני האימון של ה-AI - לא נעשה חיפוש רשת. המודל הנוכחי אינו תומך ב-Google Search grounding.',
        fiscalSourceBtlRates: 'ביטוח לאומי - סכומי קצבת אזרח ותיק {year}',
        fiscalSourceBtlIncomeTest: 'ביטוח לאומי - מבחן הכנסות מעבודה {year}',
        fiscalSourceTaxBooklet: 'רשות המסים - לוח ניכויים חודשי {year}',
        viewTaxBrackets: 'צפה במדרגות המס',
        close: 'סגור',
        // Withdrawal Strategies
        withdrawalStrategy: 'אסטרטגיית משיכה',
        withdrawalFixed: 'סכום קבוע',
        withdrawalFourPercent: 'כלל 4%',
        withdrawalPercentage: '% מהתיק',
        withdrawalDynamic: 'דינמי',
        withdrawalFixedDesc: 'משיכה חודשית קבועה',
        variableRatesDesc: 'תשואה משתנה לפי שנה',
        withdrawalFourPercentDesc: '4% מההון ההתחלתי בשנה',
        fourPercentModeLabel: 'מצב כלל 4%',
        fourPercentModeNet: 'נטו (מקבל 4% אחרי מס)',
        fourPercentModeGross: 'ברוטו (התיק מתרוקן ב-4%)',
        fourPercentModeNetDesc: 'מקבל 4%/שנה נטו. התיק מתרוקן מהר יותר בגלל גיוס ברוטו למס.',
        fourPercentModeGrossDesc: 'התיק מושך בדיוק 4% ברוטו/שנה. אחרי מס רווחי הון מקבל פחות.',
        withdrawalPercentageDesc: 'אחוז מהיתרה הנוכחית',
        withdrawalDynamicDesc: 'מתאים לפי ביצועי השוק',
        withdrawalInterestOnly: 'ריבית בלבד',
        withdrawalInterestOnlyDesc: 'מושכת את הרווח שנוצר בכל חודש אחרי מס. זה יכול לשלם יותר מהיעד, אבל משאיר פחות צמיחה לצבירה.',
        withdrawalGuardrails: 'גדרות',
        withdrawalGuardrailsDesc: 'Guyton-Klinger: מתאים ±10% כאשר אחוז המשיכה חורג ב-±20% מהשיעור ההתחלתי',
        withdrawalVpw: 'VPW',
        withdrawalVpwDesc: 'Variable Percentage Withdrawal: יתרה ÷ חודשים שנותרו — מתאים אוטומטית לאורך החיים',
        withdrawalCape: 'CAPE',
        withdrawalCapeDesc: 'CAPE מבוסס: שיעור משיכה לפי שווי שוק (P/E שילר) — פחות כשהשוק יקר, יותר כשהשוק זול',
        withdrawalPercentageRate: 'אחוז משיכה (%)',
        netWithdrawal: 'משיכה נטו',
        // Error Boundary
        displayError: 'שגיאה בתצוגה',
        unknownError: 'שגיאה לא ידועה',
        tryAgain: 'נסה שוב',
        chunkLoadError: 'גרסה חדשה של האפליקציה זמינה — יש לרענן את הדף',
        reloadPage: 'רענן דף',

        // Bucket Strategy
        bucketStrategy: 'אסטרטגיית דליים',
        enableBuckets: 'הפעל דליים נפרדים',
        accumulationRate: 'ריבית צבירה (לפני פרישה)',
        safeRate: 'ריבית דלי בטוח (פרישה)',
        surplusRate: 'ריבית דלי עודף (פרישה)',
        safeBucketDesc: 'הון שמור להכנסה חיונית',
        surplusBucketDesc: 'הון לצמיחה/ירושה',
        safeBucketBalance: 'דלי בטוח',
        surplusBucketBalance: 'דלי עודף',

        // Life Events Timeline
        lifeEventsTimeline: 'ציר זמן אירועי חיים',
        addEvent: 'הוסף אירוע',
        editEvent: 'ערוך אירוע',
        noEventsYet: 'לא נוספו אירועי חיים עדיין. לחץ "הוסף אירוע" ליצירת אחד.',
        eventType: 'סוג אירוע',
        oneTimeIncome: 'הכנסה חד פעמית',
        oneTimeExpense: 'הוצאה חד פעמית',
        incomeChange: 'שינוי בהכנסה',
        expenseChange: 'שינוי בהוצאה',
        startDate: 'תאריך התחלה',
        endDate: 'תאריך סיום',
        amount: 'סכום',
        monthlyChange: 'שינוי חודשי',
        description: 'תיאור',
        descriptionPlaceholder: 'למשל: ירושה, רכישת רכב, העלאת שכר...',
        enableEvent: 'הפעל אירוע',
        disableEvent: 'השבת אירוע',
        deleteEvent: 'מחק אירוע',
        editEventAction: 'ערוך אירוע',
        deleteAll: 'מחק הכל',
        month: 'חודש',
        january: 'ינואר',
        february: 'פברואר',
        march: 'מרץ',
        april: 'אפריל',
        may: 'מאי',
        june: 'יוני',
        july: 'יולי',
        august: 'אוגוסט',
        september: 'ספטמבר',
        october: 'אוקטובר',
        november: 'נובמבר',
        december: 'דצמבר',
        hasEndDate: 'תאריך סיום ספציפי (בטל לקבוע)',
        positiveForIncrease: 'חיובי לעלייה (+), שלילי לירידה (-)',
        saveChanges: 'שמור שינויים',
        pleaseSelectStartDate: 'נא לבחור תאריך התחלה',
        pleaseSelectEndDate: 'נא לבחור תאריך סיום',
        pleaseEnterAmount: 'נא להזין סכום תקין',
        pleaseEnterMonthlyChange: 'נא להזין שינוי חודשי תקין',
        perMonth: '/חו׳',
        viewTimeline: 'ציר זמן',
        noEventsTimeline: 'אין אירועי חיים להצגה',
        ongoing: 'ממשיך',

        // AI Error Messages  
        errorRateLimit: 'חרגת ממגבלת הקריאות. אנא נסה שוב מאוחר יותר או השתמש במודל אחר.',
        errorInvalidApiKey: 'מפתח API לא תקין. אנא בדוק את האישורים שלך בהגדרות.',
        errorModelNotFound: 'מודל "{model}" לא נמצא עבור {provider}. אנא בחר מודל אחר.',
        errorNetwork: 'שגיאת רשת. אנא בדוק את חיבור האינטרנט שלך ונסה שוב.',
        errorJsonParse: 'כשל בפענוח תגובת ה-AI. המודל החזיר מידע לא תקין. אנא נסה שוב או השתמש במודל אחר.',
        errorMissingApiKey: 'חסר מפתח API עבור ספק: {provider}. אנא הגדר את קובץ ה-.env או הוסף מפתח API בהגדרות.',
        errorGeneric: 'חישוב AI נכשל: {error}. אנא נסה שוב או עבור למצב מתמטי.',

        // Validation Error Messages
        validationInvalidInputs: 'קלטים לא תקינים:',
        validationCurrentAgeBetween: 'גיל נוכחי חייב להיות בין 0 ל-120',
        validationRetirementStartAgeBetween: 'גיל פרישה חייב להיות בין 0 ל-120',
        validationRetirementEndAgeBetween: 'גיל סיום חייב להיות בין 0 ל-120',
        validationRetirementStartGreater: 'גיל פרישה חייב להיות גבוה או שווה לגיל הנוכחי',
        validationRetirementEndGreater: 'גיל סיום חייב להיות גבוה מגיל הפרישה',
        validationCurrentSavingsNonNegative: 'חסכונות נוכחיים לא יכולים להיות שליליים',
        validationMonthlyContributionNonNegative: 'הפקדה חודשית לא יכולה להיות שלילית',
        validationMonthlyIncomePositive: 'הכנסה חודשית רצויה חייבת להיות חיובית',
        validationAnnualReturnBetween: 'תשואה שנתית חייבת להיות בין -100% ל-100%',
        validationTaxRateBetween: 'שיעור מס חייב להיות בין 0% ל-100%',

        // Rate Limit Messages
        rateLimitMinute: 'הגבלה: קריאה אחת לדקה. אנא המתן {seconds} שניות.',
        rateLimitHour: 'הגבלה: {limit} קריאות לשעה. נסה שוב ב-{time}.',
        rateLimitDay: 'הגעת למגבלה היומית ({limit} קריאות). מתאפס מחר בחצות.',
        rateLimitReached: 'הגעת למגבלה',
        callsToday: 'קריאות היום',
        usageStats: 'שימוש AI: {today}/{dailyLimit} היום',

        // Models Manager
        manageModels: 'הגדרות',
        modelsManagerDesc: 'בדוק אילו מודלים זמינים מכל ספק AI ועדכן את הרשימה.',
        checkModels: 'בדוק מודלים',
        checking: 'בודק...',
        applyUpdates: 'החל עדכונים',
        resetToDefault: 'איפוס לברירת מחדל',
        modelsUpdated: 'המודלים עודכנו בהצלחה!',
        modelsReset: 'המודלים אופסו לברירת מחדל!',
        updatesAvailable: 'עדכונים זמינים',
        existing: 'קיימים',
        new: 'חדש',
        removed: 'הוסרו',
        hideDetails: '▼ הסתר פרטים',
        showDetails: '▶ הצג פרטים',
        newModels: 'מודלים חדשים:',
        removedModels: 'מודלים שהוסרו:',

        // Header Buttons
        switchToLightMode: 'החלף למצב בהיר',
        switchToDarkMode: 'החלף למצב כהה',
        selected: 'נבחרו',
        selectAll: 'בחר הכל',
        deselectAll: 'בטל הכל',

        // AI Insights View
        numericalResults: 'תוצאות מתמטיות',
        aiInsights: 'תובנות AI',
        budget: 'תקציב',
        budgetIncomeTarget: 'יעד הכנסה חודשית',
        budgetTotalExpenses: 'סה"כ הוצאות',
        budgetGap: 'פער',
        budgetOfTarget: 'מהיעד',
        budgetAddItem: 'הוסף פריט',
        budgetNewItem: 'פריט חדש',
        budgetAddCategory: 'הוסף קטגוריה',
        budgetCategoryName: 'שם הקטגוריה:',
        budgetAdoptTarget: 'אמץ כיעד הכנסה',
        budgetReset: 'אפס',
        budgetResetConfirm: 'לאפס את כל הסכומים לאפס?',
        budgetMonthly: 'חודשי',
        budgetAnnual: 'שנתי',
        budgetClickToEdit: 'לחץ לעריכה',
        budgetAddLoan: 'הוסף הלוואה / משכנתא',
        budgetTrack: 'מסלול',
        budgetAddTrack: 'הוסף מסלול',
        budgetEndDate: 'סיום',
        budgetLoanDone: 'הסתיים',
        getAiInsights: 'קבלת תובנות AI',
        getAiInsightsDesc: 'השתמש בבינה מלאכותית כדי לנתח את המצב הפיננסי שלך ולקבל המלצות מותאמות אישית.',
        analyzeNow: 'נתח עכשיו',
        processingInsights: 'מעבד נתונים ומייצר תובנות...',
        readinessScore: 'ציון מוכנות',
        strengths: 'נקודות חוזק',
        attentionNeeded: 'נקודות לתשומת לב',
        marketDependency: 'תלות בשוק ההון',
        recommendations: 'המלצות לפעולה',
        impact: 'השפעה',
        regenerateInsights: 'צור תובנות מחדש',
        errorDetails: 'פרטי שגיאה',
        errorHighTraffic: 'עומס גבוה (מגבלת קריאות)',
        errorHighTrafficDesc: 'המודל עמוס כרגע. נסה לבחור ב-"Gemini Flash" או המתן מספר שניות.',
        errorApiKey: 'בעיית מפתח API',
        errorApiKeyDesc: 'אנא בדוק את הגדרות המפתח שלך.',
        aiRecommendedAge: 'גיל פרישה מומלץ (AI)',
        current: 'תכנון נוכחי',
        copyEventToProfile: 'העתק אירוע לפרופיל',
        selectTargetProfile: 'בחר פרופיל יעד:',
        selectTargetProfilePlaceholder: '-- בחר פרופיל יעד --',
        performCopy: 'בצע העתקה',
        copyEventError: 'שגיאה בהעתקת האירוע',
        manual: 'ידני',
        variableRates: 'ריביות משתנות',
        invalidAge: 'גיל לא תקין',
        duplicateEventError: 'שגיאה: אירוע עם שם וסוג זהים כבר קיים בפרופיל זה',
        updateError: 'שגיאה בעדכון',
        unusualValuesWarning: 'התראה: ערכים חריגים',
        unusualValuesDesc: 'הערכים שה-AI החזיר שונים משמעותית מברירת המחדל. וודא שהם נכונים לפני שמירה.',
        invalidAiResponse: 'תגובת AI לא תקינה',
    }
};
